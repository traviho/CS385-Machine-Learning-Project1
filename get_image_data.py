	...
	for r in range(0, y_bound, y_step):
		for c in range(0, x_bound, x_step):
			if (box_number != 4):
				...
				sliding_boxes.append(resized_img)
			box_number += 1
...
def get_image_data():
	...
	np_Y_train = np.array(Y_train).reshape(1, len(Y_train))
	np_X_test = np.array(X_test)
	np_Y_test = np.array(Y_test).reshape(1, len(Y_test))

	return (np_X_train, np_Y_train, np_X_test, np_Y_test)

#### Tests ####

# face_data_arr = read_ellipse_text(FOLDER_PATH + "FDDB-fold-01-ellipseList.txt", IMAGE_PATH)
# img = gen_postive_img(*face_data_arr[0])
# images = sliding_boxes_64(img)
